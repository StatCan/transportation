	...
	var canadaLayer = svg.append("g")
		...
		loadLayers = function() {
			var done = function() {
				...
				loadOtherLayers = function() {
					var q = d3.queue(),
						layerCallback = function(error, data) {
							onError(error);
							this.data = data;
							console.log("call createAdditionalLayer")
			 ...
			 //      airports = canadaLayer.selectAll("path").select(".ON")
			...
			if (settings.additonalLayers && settings.additonalLayers.length > 0) {
				console.log("cb loadOtherLayers")		
				cb = loadOtherLayers;
			}
			...
			if (typeof settings.baseMap === "object") {
				...
				d3.json(settings.baseMap, function(error, data) {
					...
					console.log("after createBaseMap")

					myAddAirports(canadaLayer, cb);
				});
			}
		},
		...
		//CN-----------------------------------------------		
		myAddAirports = function(layer, cb) {
