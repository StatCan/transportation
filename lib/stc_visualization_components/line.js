    ...
    draw = function() {
      var sett = this.settings,
        ...
        labelY = function(d) {
          ...
          return y(mergedSettings.y.getValue.call(sett, points[points.length - 1])) - 5; //CN added - 5
        },
      ...
      // CN add chart title
      if (chartInner.empty()) {
        console.log("chartInner empty!!!")
        
      } else {
        console.log("chartInner EXISTS!!!")
        chartInner
          .append("text")
          .attr("x", (innerWidth / 2))
      ...
      if (xAxisObj.empty()) {
        ...
        xAxisObj
          .append("text")
            ...
            // .attr("dy", "-0.5em")
            .attr("dy", "3.0em") // CN added
            .attr("text-anchor", "end")
            // .text(sett.x.label);
            .text(sett.x.getLabel()); // CN added
      } else {
        // xAxisObj.select("text").text(settings.x.label);
        xAxisObj.select("text").text(sett.x.getLabel()); // CN added
      }
      xAxisObj.call(
        d3.axisBottom(x)
          .ticks(sett.x.ticks)
      ...
      if (yAxisObj.empty()) {
        ...
        yAxisObj
          .append("text")
            ...
            // .text(sett.y.label);
            .text(sett.y.getLabel()); // CN added
      } else {
        // yAxisObj.select("text").text(settings.y.label);
        yAxisObj.select("text").text(sett.y.getLabel()); // CN added
      }
      yAxisObj.call(
        d3.axisLeft(y)
          .ticks(sett.y.ticks)
