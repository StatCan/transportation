    ...
    const draw = function() {
      ...
      if (xAxisObj.empty()) {
        ...
        xAxisObj
            ...
            .text(settings.x.getLabel()); // CN added
      } else {
        // xAxisObj.select("text").text(settings.x.label);
        xAxisObj.select("text").text(settings.x.getLabel()); // CN added
      }
      xAxisObj.call(
          d3.axisBottom(x)
      ...
      if (yAxisObj.empty()) {
        ...
        yAxisObj
            ...
            .text(settings.y.getLabel()); // CN added
      } else {
        // yAxisObj.select("text").text(settings.y.label);
        yAxisObj.select("text").text(settings.y.getLabel()); // CN added
      }
      yAxisObj.call(
          d3.axisLeft(y)
