...
this.areaChart = function (svg, settings, data) {
  ...
  var drawTable = function drawTable() {
    ...
    var details = parent.select("details");
    var keys = sett.z.getKeys.call(sett, filteredData);
    var table;
    var header;
    var body;
    ...
    if (!details.empty()) {
      // details.remove();
      parent.selectAll("details").remove();
    } // if (details.empty()) {


    details = parent.append("details").attr("class", "chart-data-table"); // ----Copy Button Container ---------------------------------------------
    ...
    details.append("summary") // .attr("id", summaryId)
    .attr("id", function () {
    ...
    details.append("caption") // .text(sett.datatable.title);
    .attr("class", "wb-inv").text(sett.tableTitle);
    details.append("div").attr("id", copyButtonId);
    table = details.append("table").attr("class", "table").attr("caption", summaryId);
    ...
    for (k = 0; k < keys.length; k++) {
      dataRow.append("td").attr("headers", function (d, i) {
        ...
        if (sett.y.getText) {
          var thisValue = sett.y.getText.call(sett, d, keys[k]); // return sett.y.getText.call(sett, d, keys[k]);

          return sett.formatNum ? sett.formatNum.bind(sett)()(thisValue) : thisValue;
        }
