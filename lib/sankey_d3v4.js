d3.sankey = function() {
  ...
  function computeAbsolutePositions() {
    ...
    var l1ParentNodeNum = 0; // intl
    var l0children = findLevelChildren(l1ParentNodeNum);

    // checks relative positions of Level1 and its Level 2 children group
    if (l0children[0]) checkLayout(l0children[0], 1); // 1: USres
    if (l0children[1]) checkLayout(l0children[1], 2); // 2: nonUSres
    if (l0children[2]) checkLayout(l0children[2], 3); // 3: cdnFromUS
    if (l0children[3]) checkLayout(l0children[3], 4); // 4: cndFromOther

    console.log("")
    // var l3ParentNodeNum = 7; // USres_land
    // var l2children = ["USres_land"];
    // var l3modes = ["car", "bus", "train", "other"];
    // if (l2children[0]) checkLayout(l2children[0], l3modes);

  } // computeAbsolutePositions()


  function findLevelChildren(parentNodeNum) {
    var children = [];
  ...
  function findNodeChildren(node, parentNodeNum) {
    var children = [];
  ...
  function checkLayout(nodeName, parentNodeNum) {
    // Only checks relative positions of Level1 and its Level 2 children group
    console.log("nodeName: ", nodeName)

    // var children = findNodeChildren(nodeName, modes);
    var children = findNodeChildren(nodeName, parentNodeNum);
    ...
    var midNodeChildrenGroup = findGroupMidpt(children);

    // -------------------------------------------------------------------------
    // Check if midpoints are within reasonable y-distance (85%)
    // ACTION: move node accordingly
    console.log("midNode, midChildgrp: ", midNode, midNodeChildrenGroup)
    if ([midNode, midNodeChildrenGroup].sort(function(a,b) { return a - b;})[0] /
        [midNode, midNodeChildrenGroup].sort(function(a,b) { return a - b;})[1] < 0.90) {      
     
      if (midNode > midNodeChildrenGroup) { // L_A node should be moved up
