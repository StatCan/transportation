d3.sankey = function() {
  ...
  function computeAbsolutePositions() {
        ...
        // Check if Level 2 children exist (Air, Marine, Land)
        var childAir; // store child if exists
        var childMarine;
        var childLand;
        // Air
        if (nodes.filter(function (item) {
          ...
          childAir =  nodes.filter(function (item) {
              return item.name === "USres_air";
          })[0];
        }
        // Marine
        if (nodes.filter(function (item) {
          ...
          childMarine =  nodes.filter(function (item) {
              return item.name === "USres_marine";
          })[0];
        }
        // Land
        if (nodes.filter(function (item) {
          ...
          childLand =  nodes.filter(function (item) {
              return item.name === "USres_land";
          })[0];
        }
        ...
        midLhat_A = Lhat_A0 + ( Lhat_A1 - Lhat_A0 ) / 2;
        ...
        if ([midL_A, midLhat_A].sort(function(a,b) { return a - b;})[0] /
          ...
          if (L_A0 > midLhat_A) { // L_A node should be moved up
            ...
            console.log("child move down: ", childAir)
            if (childAir) childAir.y = childAir.y - midL_A/3;
            if (childMarine) childMarine.y = childMarine.y - midL_A/3;
            if (childLand) childLand.y = childLand.y - midL_A/3;
          }
        }
  }
  //
