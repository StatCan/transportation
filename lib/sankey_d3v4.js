d3.sankey = function() {
  ...
  function computeAbsolutePositions() {
    ...
    var l0children = findLevel0Children(intl1node.name);

    // checks relative positions of Level1 and its Level 2 children group
    if (l0children[0]) checkLayout(l0children[0]);
    if (l0children[1]) checkLayout(l0children[1]);
    if (l0children[2]) checkLayout(l0children[2]);
    if (l0children[3]) checkLayout(l0children[3]);

  } // computeAbsolutePositions()

  function findLevel0Children() {
    var children = [];

    nodes.map(function (item) {
      if (item.targetLinks.length !== 0 && item.targetLinks[0].source.node === 0) {
  ...
  function findLevel1Children(l1node) {
  ...
  function checkLayout(nodeName) {
    ...
    var midNodeChildrenGroup = findGroupMidpt(l1children);
