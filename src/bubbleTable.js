var bubbleTable = (function () {
	...
	var defaults = {
	  ...
	  aspectRatio: 16 / 9,
	  x: {
	    getDomain: function getDomain(flatData) {
	      var _this = this;

	      if (!this.x.type || this.x.type === "ordinal") {
	        return d3.set(flatData, function () {
	          for (var _len = arguments.length, args = new Array(_len), _key = 0; _key < _len; _key++) {
	            args[_key] = arguments[_key];
	          }

	          return _this.x.getValue.apply(_this, args);
	        }).values();
	      }

	      return d3.extent(flatData, this.x.getValue.bind(this));
	    },
	    getRange: function getRange(d, domain) {
	      if (!this.x.type || this.x.type === "ordinal") {
	        var arr = [];
	    ...
	    getDeltaX: function getDeltaX(flatData) {
	      var _this2 = this;

	      if (!this.x.type || this.x.type === "ordinal") {
	        var thisList = d3.set(flatData, function () {
	          for (var _len2 = arguments.length, args = new Array(_len2), _key2 = 0; _key2 < _len2; _key2++) {
	            args[_key2] = arguments[_key2];
	          }

	          return _this2.x.getValue.apply(_this2, args);
	        }).values();
	        return thisList[1];
	      }
	    }
	  ...
	  r: {
	    getDomain: function getDomain(flatData) {
	      return [d3.min(flatData, this.r.getValue.bind(this)), d3.max(flatData, this.r.getValue.bind(this))];
	    },
	    getRange: function getRange(flatData) {
	...
	var bubbleTable = window.bubbleTable = function (svg, settings, data) {
	  ...
	  var draw = function draw() {
	    ...
	    var flatData = (_ref = []).concat.apply(_ref, _toConsumableArray(filteredData.map(function (d) {
	    ...
	    var xDomain = sett.x.getDomain.call(sett, flatData); // Mapping for x-coord of table based on year

	    x = rtnObj.x = xScale.domain(xDomain).range(sett.x.getRange.call(sett, flatData, xDomain)); // Mapping for z-coord of table based on id

	    z = rtnObj.z = d3.scaleLinear().domain(sett.z.getDomain.call(sett, filteredData)).range(sett.z.getRange.call(sett, filteredData)); // Mapping for circle radius based on rank
	    ...
	    var bubbleText = (sett.r.getText ? sett.r.getText : sett.r.getValue).bind(sett);

	    if (dataLayer.empty()) {
	      dataLayer = chartInner.append("g").attr("class", "data");
	    }

	    var bubbleGroups = dataLayer.selectAll(".bubble-group").data(filteredData, sett.z.getId.bind(sett));
	    bubbleGroups.enter().append("g").attr("id", sett.z.getId.bind(sett)).attr("class", sett.z.getId.bind(sett)).attr("transform", function () {
	      for (var _len3 = arguments.length, args = new Array(_len3), _key3 = 0; _key3 < _len3; _key3++) {
	        args[_key3] = arguments[_key3];
	      }

	      var zVal = z(sett.z.getValue.apply(this, args)) + deltaZ / 2;
	      return "translate(0, ".concat(zVal, ")");
	    }).each(function () {
	      // d is each z
	      var group = d3.select(this);

	      for (var _len4 = arguments.length, dArgs = new Array(_len4), _key4 = 0; _key4 < _len4; _key4++) {
	        dArgs[_key4] = arguments[_key4];
	      }

	      var bubbles = group.selectAll(".bubble").data(sett.z.getDataPoints.apply(this, dArgs)); // Create bubble g node and attach data

	      var newBubbles = bubbles.enter().append("g").attr("class", "bubble").attr("transform", function () {
	        for (var _len5 = arguments.length, args = new Array(_len5), _key5 = 0; _key5 < _len5; _key5++) {
	          args[_key5] = arguments[_key5];
	        }

	        // d is each point
	        var xVal = x(sett.x.getValue.apply(this, args)) + deltaX / 2;
	        return "translate(".concat(xVal, ", 0)");
	      }); // Append circles to bubble g node

	      newBubbles.append("circle").attr("r", function () {
	        for (var _len6 = arguments.length, args = new Array(_len6), _key6 = 0; _key6 < _len6; _key6++) {
	          args[_key6] = arguments[_key6];
	        }

	        var rVal = r(sett.r.getValue.apply(sett, args));
	        return rVal;
	      }); // Append bubble text to bubble g node

	      newBubbles.append("text").attr("dx", function (d) {
	        return 0 + shiftColTextX; // 0;
	      }).attr("dy", function (d) {
	        return 0;
	      }).attr("class", "bubble-text").text(function (d) {
	        return format(bubbleText(d));
	      }).attr("dominant-baseline", "central"); // Label rows by z.id

	      group.append("text").attr("dx", function (d) {
	        return -1 * mergedSettings.margin.left;
	      }).attr("dy", 0).attr("class", "bubble-row-text").text(function () {
	    ...
	    xAxisObj.call(d3.axisTop(x).tickSizeOuter(0) // .ticks(sett.x.ticks)
