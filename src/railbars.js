(function () {
  ...
  var settings = {
    ...
    filterData: function filterData(data) {
      ...
      var keys = Object.keys(root[0]).slice(1); // exclude first key (year is for x-axis, not line labels)

      var rtn = keys.map(function (d) {
        return {
          id: d,
          data: root.map(function (p) {
            return {
              year: p.year,
              value: p[d]
            };
  ...
  function createLegend (regPairs, legendID) {
    ...
    .attr("transform", "translate(" + 345 + "," + 0 + ")"); // Create the g nodes

    var lines = svg.selectAll("rect").data(regPairs).enter().append("g"); // Append lines onto the g nodes and fill
  ...
  var targetRegion = "AT"; // reciprocal region for line chart pair

  var domain; // Stores domain of flattened json1

  var scalef = 1e3; // scale factor for data values
  // const regions = ["AT", "QC", "ON", "MB", "SK", "AB", "BC", "US-MEX"];

  var regions = ["AT", "ON", "QC", "MB", "SK", "AB", "BC"];

  var getRemainingRegions = function getRemainingRegions() {
    return regions.filter(function (item) {
      return item !== selectedRegion;
    });
  ...
  var chartPair1 = d3.select("#pair1").append("svg").attr("id", "svg_pair1");
  var barSVG = [];

  for (var c = 0; c < getRemainingRegions().length; c++) {
    barSVG.push(d3.select("#bar".concat(c)).append("svg").attr("id", "svg_bar".concat(c)));
  } // barChart title divs


  var titleDIV = [];

  for (var _c = 0; _c < getRemainingRegions().length; _c++) {
    titleDIV.push("#bar".concat(_c, "Title"));
  } // ---------------------------------------------------------------------
  ...
  function filterDataBar(d) {
    ...
    return [{
      category: "".concat(this.selectedRegion, "to").concat(this.targetRegion),
      values: Object.keys(d).map(function (p) {
        return {
          year: p,
          value: d[parseInt(p, 10)][_this.targetRegion] / scalef
        };
      })
    }];
  }
  ...
  function updateTitles(idx) {
    ...
    var comm = i18next.t(selectedComm, {
      ns: "commodities"
    }); // Title for group of bar charts

    d3.select("#barChartTitle").text("".concat(comm, " transported by rail from ").concat(geography, " to all destinations")); // individual bar chart titles

    var targetGeo = i18next.t(getRemainingRegions()[idx], {
      ns: "railGeography"
    });
    d3.select(titleDIV[idx]).text(targetGeo);
  } // ---------------------------------------------------------------------
  ...
  i18n.load(["src/i18n"], function () {
    ...
    d3.json("data/rail/" + selectedRegion + "_" + selectedComm + ".json", function (err, json1) {
      ...
      var remainingRegions = getRemainingRegions();
      domain = [0, 65];
      ...
      settingsBar.y.getDomain = function () {
        return domain;
      };

      for (var idx = 0; idx < remainingRegions.length; idx++) {
        var _targetRegion = remainingRegions[idx]; // barChart(eval(`bar${idx}`), {...s, selectedRegion, targetRegion}, json1);

        var thisSVG = barSVG[idx];
        barChart(thisSVG, _objectSpread({}, s, {
          selectedRegion: selectedRegion,
          targetRegion: _targetRegion
        }), json1); // rotate x-axis labels

        d3.selectAll(".railBar").selectAll(".x.axis").selectAll(".tick").selectAll("text").attr("transform", "rotate(-45)").style("text-anchor", "end");
        updateTitles(idx);
      }

      d3.json("data/rail/" + targetRegion + "_" + selectedComm + ".json", function (err, json2) {
        var numYears = Object.keys(json1).length; // Construct data object pair to send to stacked area chart

        var arrPair = [];

        for (var _idx = 0; _idx < numYears; _idx++) {
          var _arrPair$push;

          var thisYear = Object.keys(json1)[_idx];

          arrPair.push((_arrPair$push = {
            year: thisYear
          }, _defineProperty(_arrPair$push, selectedRegion + "to" + targetRegion, json1[thisYear][targetRegion]), _defineProperty(_arrPair$push, targetRegion + "to" + selectedRegion, json2[thisYear][selectedRegion]), _arrPair$push));
        }
