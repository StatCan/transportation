(function () {
  ...
  var chartPair2 = d3.select("#pair2").append("svg").attr("id", "svg_pair2");
  ...
  i18n.load(["src/i18n"], function () {
    ...
    d3.json("data/rail/" + selectedComm + "_" + selectedRegion + ".json", function (err, json1) {
      ...
      var _loop = function _loop(idx) {
        var thisReg = remainingRegions[idx];
        d3.json("data/rail/" + selectedComm + "_" + thisReg + ".json", function (err, json2) {
          data[thisReg] = json2; // Construct data object pair to send to stacked area chart
          ...
          for (var _idx = 0; _idx < numYears; _idx++) {
            ...
            arrPair.push((_arrPair$push = {
              year: data[thisReg][_idx].year
            }, _defineProperty(_arrPair$push, selectedRegion + "to" + thisReg, data[selectedRegion][_idx][thisReg]), _defineProperty(_arrPair$push, thisReg + "to" + selectedRegion, data[thisReg][_idx][selectedRegion]), _arrPair$push));
          } // send to lineChart
