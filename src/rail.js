(function () {
	...
	var vertical = d3.select("#annualTimeseries").append("div").attr("class", "remove").style("position", "absolute").style("z-index", "19").style("width", "2px").style("height", "500px").style("top", "10px").style("bottom", "30px").style("left", "0px").style("background", "#ccc"); // TEMPORARY HACK UNTIL CAN OBTAIN AREACHART XSCALE
	  ...
	  d3.json("data/rail/rail_" + selectedComm + "_orig" + selected + "_all_dest.json", function (err, fileorig) {
	    ...
	    d3.json("data/rail/rail_" + selectedComm + "_all_orig_to_" + selected + ".json", function (err, filedest) {
	      ...
	      d3.select("#nextButton").on("click", function () {
	        count++;
	        count === 0 ? d3.select("#prevButton").classed("inactive", true) : d3.select("#prevButton").classed("inactive", false); // drawBubbles(rankedCommData, rankedCommNames, years, maxVal, count);
	      });
